...
from myadmin.models import Types,Goods,Detail,Orders
...
def goodsupdate(request,gid):
    try:
        ...
        if b:
            ...
            os.remove("./static/goods/XL_"+oldpicname) #执行老图片删除
            os.remove("./static/goods/L_"+oldpicname) #执行老图片删除
        ...
        if b:
            ...
            os.remove("./static/goods/L_"+picname) #执行新图片删除
            os.remove("./static/goods/XL_"+picname) #执行新图片删除
...
#浏览订单状态页
def ordersedit(request,orid):
    ob = Orders.objects.get(id=orid)
    context = {'orders':ob}
    return render(request,"myadmin/orders/ordersedit.html",context)
    
#执行订单状态编辑
def ordersdoedit(request,orid):
    try:
        ob = Orders.objects.get(id=orid)
